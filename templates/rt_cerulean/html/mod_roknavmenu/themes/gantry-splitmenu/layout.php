<?php
/**
* @version   $Id: layout.php 22786 2014-08-25 19:47:08Z james $
...
class GantrySplitmenuLayout extends AbstractRokMenuLayout
{
    ...
    public function stageHeader()
    {
        ...
        if (!self::$jsLoaded && $gantry->get('layout-mode', 'responsive') == 'responsive'){
            if (!($gantry->browser->name == 'ie' && $gantry->browser->shortver < 9)){
                ...
                if ($this->args['responsive-menu'] == 'selectbox') {
                    $gantry->addScript($gantry->baseUrl . 'templates/rt_cerulean/html/mod_roknavmenu/themes/default/js/responsive.js');
                    $gantry->addScript($gantry->baseUrl . 'templates/rt_cerulean/html/mod_roknavmenu/themes/default/js/responsive-selectbox.js');
                } else if (file_exists($gantry->basePath . '/templates/rt_cerulean/html/mod_roknavmenu/themes/default/js/sidemenu.js') && ($this->args['responsive-menu'] == 'panel')) {
                    $gantry->addScript($gantry->baseUrl . 'templates/rt_cerulean/html/mod_roknavmenu/themes/default/js/sidemenu.js');
                }
            }
            self::$jsLoaded = true;
        }
        ...
        if ($gantry->browser->name == 'ie' && $gantry->browser->shortver < 9){
            $gantry->addLess('menu-hovers.less', 'menu-hovers.css', 1, array('main-accent'=>$gantry->get('main-accent')));
        }
    }
